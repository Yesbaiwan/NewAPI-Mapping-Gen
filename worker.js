...
const jsonResponse = (data, status = 200) => new Response(JSON.stringify(data), {
    status, headers: { ...HEADERS.json, ...HEADERS.cors }
});
...
export default {
    async fetch(request) {
        ...
        return handlers[new URL(request.url).pathname]?.() ?? new Response('Not Found', { status: 404, headers: HEADERS.cors });
    }
};

const buildFullUrl = (url, path) => `${url.replace(/\/+$/, '')}/${path.replace(/^\/+/, '')}`;

const filterModels = (ids, keywords, mode) => {
    const kw = keywords?.split(",").map(k => k.trim().toLowerCase()).filter(Boolean);
    if (!kw?.length) return ids;
    return ids.filter(id => kw.some(k => id.toLowerCase().includes(k)) !== (mode === "exclude"));
};

async function parseRequestBody(request) {
    const body = await request.json().catch(() => null);
    if (!body) {
        throw Object.assign(new Error('请求体必须是有效的JSON'), { status: 400 });
    }
    return body;
}
...
async function handleFetch(request) {
    try {
        ...
        const { apiUrl, apiEndpoint = "/v1/models", apiKey, enableFilter, filterKeywords, filterMode } = body;
        ...
        const modelIds = (Array.isArray(json.data) ? json.data : []).map(m => m.id).filter(Boolean).sort();
        if (!modelIds.length) throw new Error("未找到模型ID");

        const filtered = enableFilter ? filterModels(modelIds, filterKeywords, filterMode) : modelIds;
        return jsonResponse({ allModels: filtered, filteredModels: filtered });
    } catch (error) {
...
async function handleGenerateMapping(request) {
    try {
        ...
        const mapping = Object.fromEntries(
            finalModels.map((f, i) => [f, allModels[i]]).filter(([f, a]) => f && a && f !== a)
        );

    ...
    const handlers = {
      "/": () =>
        new Response(htmlContent, {
          headers: { ...HEADERS.html, ...HEADERS.cors },
        }),
      "/fetch": () => handleFetch(request),
      "/generate-mapping": () => handleGenerateMapping(request),
    };
    return (
      handlers[new URL(request.url).pathname]?.() ??
      new Response("Not Found", { status: 404, headers: HEADERS.cors })
    );
  },
};

const buildFullUrl = (url, path) =>
  `${url.replace(/\/+$/, "")}/${path.replace(/^\/+/, "")}`;

const filterModels = (ids, excludeKeywords, includeKeywords, mode) => {
  if (mode === "exclude") {
    ...
    return ids.filter((id) => !kw.some((k) => id.toLowerCase().includes(k)));
  } else {
    const kw = includeKeywords
      ?.split(",")
      .map((k) => k.trim().toLowerCase())
      .filter(Boolean);
    if (!kw?.length) return ids;
...
async function handleFetch(request) {
  try {
    ...
    const filtered = enableFilter
      ? filterModels(modelIds, excludeKeywords, includeKeywords, filterMode)
      : modelIds;
    return jsonResponse({
      allModels: filtered,
...
async function handleGenerateMapping(request) {
  try {
    ...
    if (allModels.length !== finalModels.length) {
      return jsonResponse(
        {
          error: `模型数量不一致: 源模型 ${allModels.length} 个, 最终模型 ${finalModels.length} 个`,
        },
        400,
      );
    }
